...
// ── Dynamic per-message trimming based on distance from end ──────────────────
function trimMessagesDynamic(messages) {
  ...
  return messages.map(function(msg, i) {
    ...
    // Always keep the last message fully intact
    if (i === total - 1) return msg;

    const distanceFromEnd = total - 1 - i;

    let maxChars;
    if      (distanceFromEnd <= 2)  maxChars = 3000;
    else if (distanceFromEnd <= 6)  maxChars = 1500;
    else if (distanceFromEnd <= 15) maxChars = 600;
    else                            maxChars = 200;
